...
def _asset_record(path: Path, root: Path) -> dict[str, Any]:
    ...
    return {"path": relative, "category": _classify(path), "media_type": mimetypes.guess_type(path.name)[0] or "application/octet-stream", "size_bytes": path.stat().st_size, "sha256": digest}
...
def ingest_directory(source_dir: str | Path, *, title: str | None = None, author: str = "Unknown") -> dict[str, Any]:
    ...
    if not root.is_dir(): raise ValueError(f"Source directory not found: {source_dir}")
    ...
    for asset in assets:
        if asset["path"].lower().endswith(".csv"):
            ...
            if summary: csv_summaries[asset["path"]] = summary
    text_sources = []
    for asset in assets:
        path = root / asset["path"]
        if path.suffix.lower() in {".txt", ".md", ".json", ".py"}:
            text = _read_text(path)
            if text: text_sources.append({"path": asset["path"], "preview": text[:2000]})

    twin = {
        "$schema": "https://twinth.ink/schemas/twin.v0.1.json",
        "identity": {"twin_id": "twin_draft", "slug": slug, "title": stem, "summary": "First-pass Twin assembled from supplied source material.", "version": "0.1.0", "author": {"name": author}, "license": "UNSPECIFIED", "status": "DRAFT"},
        "reality_state": {"composite_score": 0, "derived_dimensions": {}},
        "claims": [],
        "object": {"primary_3d": next((p for p in cad if p.lower().endswith(".glb")), None), "cad_source": next((p for p in cad if p.lower().endswith((".step", ".stp"))), None), "layers": []},
        ...
        "unknowns": [{"id": "unk_ingestion_01", "category": "Ingestion", "issue": "No human-reviewed technical claims have been established by the ingestion pass."}],
        "_ingestion": {"asset_count": len(assets), "assets": assets, "csv_summaries": csv_summaries, "text_sources": text_sources, "detected": {"cad": cad, "data": data, "simulation": simulation, "images": images}},
    }
    ...
    if validation is not True: raise ValueError(validation)
    return twin

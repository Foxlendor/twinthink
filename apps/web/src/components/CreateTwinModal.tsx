...
import { X, UploadCloud, FileText, CheckCircle2, Loader2, ArrowRight } from 'lucide-react';
...
export default function CreateTwinModal({ onClose }: CreateTwinModalProps) {
  ...
  const steps = [
    ...
    'Organizing evidence',
    'Creating history',
    'Assembling living digital twin'
  ];
  ...
  const handleStartBuilding = () => {
    ...
    // Simulate quiet processing sequence
    let current = 0;
        ...
        {!isProcessing ? (
          <>
            ...
            <p style={{ fontSize: '0.9375rem', color: '#4B5563', margin: '0 0 2rem 0', lineHeight: 1.5 }}>
              Turn your idea into a living digital record.
            </p>
          ...
          <div style={{ padding: '2rem 1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', textAlign: 'left', maxWidth: '280px', margin: '0 auto' }}>
              {steps.map((step, idx) => (
                <div key={idx} style={{
                  display: 'flex',
                  alignItems: 'center',
